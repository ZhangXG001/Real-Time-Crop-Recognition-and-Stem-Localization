...
parser.add_argument('--input_dir',
                    type=str,
                    default=r'/home/AirsZXG/Bonnetstem/dataset/test/')
parser.add_argument('--model_dir',
                    type=str,
                    default=r'./modelfuse3')#./model1
parser.add_argument('--save_dirseg',
                    type=str,
                    default=r'./resultfuse3/seg')#./result1
parser.add_argument('--save_dirstem',
                    type=str,
                    default=r'./resultfuse3/stem')#./result1               
...
#    print('shape of X', X.get_shape())
    ...
    predseg = tf.get_collection('seg')[0]    # 从集合'upscore_fuse'中读取第一个元素，也就是side output融合层的输出
    predstem = tf.get_collection('stem')[0]
    '''
...
#        print(image_feed.get_shage())
        ...
        if num_images == batch_size:
            start2=time.time()
            label_predseg, label_predstem = sess.run([predseg,predstem], feed_dict={X: image_feed, training: False})    # 喂入数据，计算输出
            end2=time.time()
            print(end2-start2)
            ...
            for i in range(label_predseg.shape[0]):
                mergedseg = np.squeeze(label_predseg[i])    #删除label_pred张量中长度为1的维度，即变成一个二维矩阵
                mergedstem = np.squeeze(label_predstem[i])
                mergedseg=np.uint8((mergedseg)*255)
                mergedstem=np.uint8((mergedstem)*255)
                
                crt_name = names[num_batchs*batch_size+i]    # 获取当前图片的文件名
                save_nameseg = os.path.join(flags.save_dirseg, crt_name[:-4]+".png")    # 组合成保存处理结果的路径
                save_namestem = os.path.join(flags.save_dirstem, crt_name[:-4]+".png")    # 组合成保存处理结果的路径
                cv2.imwrite(save_nameseg, mergedseg)    # 保存图片到直径路径下
                cv2.imwrite(save_namestem, mergedstem) 
                print('Pred saved')
